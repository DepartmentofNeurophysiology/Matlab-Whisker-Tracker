...
%x1_Janelia = find(General.keep, 1, 'first');
%x2_Janelia = find(General.keep, 1, 'last');
...
hx = size(Janelia.Objects,1);
hy = size(Janelia.Objects,2);
...
%{
% Generate colormap
if length(General.Janelia_labels) <= 6
    cc(1,:) = cmap1(2,:);
    cc(2,:) = cmap1(10,:);
    cc(3,:) = cmap2(10,:);
    cc(4,:) = cmap3(2,:);
...
if printJanelia
    ...
    for i = 1:nframes
        ...
        ntraces = size(Janelia.Parameters{i},1);
        
        if ntraces > 0
            pos_xo(end+1:end+ntraces,1:2) = [ones(ntraces,1)*i, Janelia.Parameters{i}(:,1)];
            pos_yo(end+1:end+ntraces,1:2) = [ones(ntraces, 1)*i, Janelia.Parameters{i}(:,2)];
            pos_xoc(end+1:end+ntraces,1:2) = [ones(ntraces, 1)*i, Janelia.Parameters{i}(:,9)];
            pos_yoc(end+1:end+ntraces,1:2) = [ones(ntraces, 1)*i, Janelia.Parameters{i}(:,10)];
            pos_xt(end+1:end+ntraces,1:2) = [ones(ntraces,1)*i, Janelia.Parameters{i}(:,3)];
            pos_yt(end+1:end+ntraces,1:2) = [ones(ntraces, 1)*i, Janelia.Parameters{i}(:,4)];
            pos_xtc(end+1:end+ntraces,1:2) = [ones(ntraces,1)*i, Janelia.Parameters{i}(:,11)];
            pos_ytc(end+1:end+ntraces,1:2) = [ones(ntraces, 1)*i, Janelia.Parameters{i}(:,12)];
            theta(end+1:end+ntraces,1:2) = [ones(ntraces, 1)*i, Janelia.Parameters{i}(:,5)];
            thetac(end+1:end+ntraces,1:2)= [ones(ntraces, 1)*i, Janelia.Parameters{i}(:,6)];
            nose_x(end+1,1:2) = [i Janelia.Parameters{i}(1,14)]; %#ok<*SAGROW>
            nose_y(end+1,1:2) = [i Janelia.Parameters{i}(1,15)];
            theta_h(end+1,1:2) = [i Janelia.Parameters{i}(1,13)];
        end
