...
%     ./Settings.Trace_kernel_large^2, 'same'); %  % Box kernel filtered
% Delta_kernel = conv2(Frame, ones(Settings.Trace_kernel_small,Settings.Trace_kernel_small)...
%     ./Settings.Trace_kernel_small^2, 'same');  % Delta kernel filtered

kernel = -ones(Settings.Trace_kernel_large, Settings.Trace_kernel_large)./Settings.Trace_kernel_large.^2;
midx = ceil(Settings.Trace_kernel_large/2) - (Settings.Trace_kernel_small - 1);
kernel(midx:midx+Settings.Trace_kernel_small-1, midx:midx+Settings.Trace_kernel_small-1) = 1;
...
Control = zeros(size(Frame));
Control( conv2(Frame, kernel, 'same') < Settings.Trace_threshold) = 1;
